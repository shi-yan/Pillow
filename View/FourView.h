...
public:
	...
	void onPaint();
	bool onLeftDown(unsigned int x,unsigned int y);
	...
	{
		//这里要判断一下鼠标在哪个区域中
		if(selected==0 && x<splitX && y<splitY)
		{
			camera[selected]->onPanPress(x,y);
		}
		else if(selected==2 && x<splitX && y>splitY)
		{
			camera[selected]->onPanPress(x,y);
		}
		else if(selected==1 && x>splitX && y<splitY)
		{
			camera[selected]->onPanPress(x,y);
		}
		else if(selected==3 && x>splitX && y>splitY)
		{
			camera[selected]->onPanPress(x,y);
		}
		return true;
	...
	void update(size_t theSplitX,size_t theSplitY,size_t theWidth,size_t theHeight)
	{
		...
		width=theWidth;
		height=theHeight;
		camera[2]->updateSize(0.0f,0.0f,(float)splitX,(float)splitY);
		camera[0]->updateSize(0.0f,(float)splitY,(float)splitX,(float)(height-splitY));
		camera[3]->updateSize((float)splitX,0.0f,(float)(width-splitX),(float)splitY);
		camera[1]->updateSize((float)splitX,(float)splitY,(float)(width-splitX),(float)(height-splitY));
	};
