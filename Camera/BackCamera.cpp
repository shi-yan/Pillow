...
void BackCamera::setCameraForSelectionD(size_t x1,size_t y1,size_t x2,size_t y2,size_t h)
{
    glViewport((GLint)m_startX,(GLint)m_startY,(GLint)m_width,(GLint)m_height);
    GLint viewport[4];
    ...
    void BackCamera::setReferenceImage(const char *path,GLuint imageID,Vector &position,size_t width,size_t height)
{
        if(backReference)
        {
...
void BackCamera::getViewportImage(GLuint &texID,std::string &path,Vector &position,size_t &width,size_t &height)
{
    if(backReference)
    {
